...
internal class DataService
{
    public static ObservableCollection<TreeModel> GetData()
    {
        ...
        for (int i = 1; i <= 3; i++)
        {
            TreeModel node = new TreeModel() { DisplayText = string.Format("Item {0}", i), SelectedValue = Guid.NewGuid() };

            for (int j = 1; j <= 3; j++)
            {
                TreeModel child = new TreeModel() { DisplayText = string.Format("Item {0}.{1}", i, j), SelectedValue = Guid.NewGuid() };
                node.AddChild(child);
